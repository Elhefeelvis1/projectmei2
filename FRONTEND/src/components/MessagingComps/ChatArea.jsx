...
import { SendHorizontal, ArrowLeft, Package, AlertCircle, Lock } from 'lucide-react';
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <main className="flex-1 p-4 overflow-y-auto bg-gray-50 space-y-4">
        {messages.map((msg) => {
          ...
          // 1. INTERCEPT SYSTEM MESSAGES HERE
          if (msg.is_system) {
            return (
              <div key={msg.id} className="flex justify-center my-6">
                <div className="bg-white border border-gray-200 shadow-sm px-5 py-2.5 rounded-2xl flex items-center gap-2 max-w-[85%] sm:max-w-[70%]">
                  <Lock size={14} className="text-gray-400" />
                  <span className="text-xs text-gray-600 font-medium leading-relaxed text-center">
                    {msg.text}
                  </span>
                </div>
              </div>
      ...
      {activeConversation.is_open === false ? (
        <footer className="px-4 py-3 border-t border-gray-200 bg-gray-50 flex items-center justify-center gap-2">
          <Lock size={14} className="text-gray-400 shrink-0" />
          <p className="text-xs text-gray-500 text-center">
            This conversation is closed because the pickup was completed.
          </p>
        </footer>
      ) : (
