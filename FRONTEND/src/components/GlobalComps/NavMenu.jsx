...
import { User, ListTodo, Gavel, Package, Heart, Settings, HelpCircle, LogOut, LogIn, Bell } from "lucide-react";
    ...
    <>
      ...
      {isOpen && (
        <>
          ...
          <div className="fixed right-4 top-20 z-[120] w-64 bg-white rounded-2xl shadow-xl border border-gray-100 py-3 animate-in fade-in zoom-in duration-200 overflow-hidden">
            {currentView === "menu" ? (
              <nav className="flex flex-col">
                {session && (
                  <>
                    ...
                    <button onClick={() => setCurrentView("notifications")} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-50 text-gray-700 transition-colors text-left cursor-pointer border-none bg-transparent">
                      <Bell size={18} />
              ...
              <NotificationsPanel session={session} onBack={() => setCurrentView("menu")} />
            ) : null}
          </div>
        </>
      )}
